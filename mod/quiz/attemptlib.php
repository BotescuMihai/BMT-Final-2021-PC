...
class quiz_attempt {
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if($_SESSION["prev_mark"]  == $this->quba->get_total_mark() && $boolean == true){
            $_SESSION["lives_no"] = $_SESSION["lives_no"] - 1;
            error_log('am pierdut o viata'); 
            error_log('Numar vieti:' . $_SESSION["lives_no"]);// die();
        }
        error_log('Prev mark:' . $_SESSION["prev_mark"] ); 
;
    $_SESSION["prev_mark"] = $this->quba->get_total_mark();
    error_log('this->quba : ' . $this->quba->get_total_mark());
